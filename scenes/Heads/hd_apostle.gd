...
func update_desc() -> void:
	if invert_logic:
		description = tr("HD_APOSTLE_DESC_ELITE") % [Constants.hd_apostle_corruption_to_hero, Constants.hd_apostle_health_decrement]
	else:
		match level:
			0:
...
func apply_passive_effect() -> void:
	if invert_logic:
		ActionManager.add(ChangeMaxHpAction.new(self, Global.enemy, -Constants.hd_apostle_health_decrement))
	else:
